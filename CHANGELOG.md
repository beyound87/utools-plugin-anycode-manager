...
1. 新增插件内直接对话功能，支持 Claude Code/Codex/Gemini/OpenCode 四平台
2. Claude Code 走持久进程 stream-json 双向协议，流式打字机增量渲染
3. Codex/Gemini/OpenCode 走一次一进程模式，整段回复渲染
4. 支持粘贴图片发送（Claude base64 原生，其他 CLI 临时文件传递）
5. 支持添加文件/文件夹附件（@路径注入）
6. 新增新建会话功能，默认当前会话目录，支持切换工作目录
7. 新增模型选择器：探测 base_url/v1/models 自动列出可用模型，失败回退常见预设+手动输入
8. 新增 effort 选择器（low/medium/high/max），Codex/OpenCode 支持
9. 权限模式选择器（只读/允许编辑/全自动），默认只读安全
10. 流式渲染 100ms 节流，避免高频 token 更新卡顿
11. 进程生命周期管理：切会话/退插件自动停止，身份校验防竞态
12. 工具调用/结果在对话中实时渲染工具卡片

## v1.1.0

1. 新增跨会话全文搜索：搜索框切换全文模式，搜索所有会话内容，点击结果打开并高亮定位
2. 新增统计洞察面板：会话数/占用空间/平台分布/最近14天活跃度/Top项目/OpenCode 花费与 Token
3. 重命名和收藏全平台支持：改用 dbStorage 存储，四平台统一，Claude 老数据仍兼容
4. 新增会话列表自动刷新：每 8s 轮询，其他 CLI 新建会话自动出现（含 OpenCode）
5. 新增记住上次打开的会话，重开插件自动恢复
6. 新增一键展开/折叠全部工具调用
7. 新增键盘上下键在会话间导航、Ctrl+K 快速全文搜索
8. 新增侧边栏右缘拖拽调宽，宽度记忆
9. 新增 uTools 主搜索框直达最近会话（onMainPush）
